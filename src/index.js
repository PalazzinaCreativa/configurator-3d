    ...
    el
      .addEventListener('click', async () => {
        ...
        if (el.getAttribute('data-type') === 'Model' || el.getAttribute('data-type') === 'Base') {
          ...
          if (document.querySelector('[data-type=Base].active').getAttribute('data-finish') === 'texture') {
            document.querySelector('.colors').style.display = 'none'
            document.querySelector('.textures').style.display = 'block'
          } else if (document.querySelector('[data-type=Base].active').getAttribute('data-finish') === 'colors') {
          ...
          el.classList.add('active')
          if (el.getAttribute('data-roughness')) {
            viewer.updateMaterial('_change_color_', {
              roughness: el.getAttribute('data-roughness')
            })
          }

          // viewer.updateTexture('_change_color_', el.getAttribute('data-color') || '')
        } else if (el.getAttribute('data-type') === 'Texture') {
          const texture = `/textures/${el.getAttribute('data-texture')}`
